...
class Table extends API{
  ...
  public function config($name, $content, $plugin = null){
    switch ($name) {
			...
			case "division":
				...
				if((isset($content))&&(!empty($content))){
					foreach($divisions as $division){
						$result = $this->Database->get('divisions',$division)->fetchArray();
      ...
      case "updated_by":
				...
				if( strpos($content, ',') !== false ) {
					...
					foreach(explode(',',trim($content,',')) as $user){
						$result = $this->Database->get('users',$user)->fetchArray();
						if(!empty($result)){
					...
					$result = $this->Database->get('users',$content)->fetchArray();
      ...
      case "clients":
        $config['type'] = 'text';
				$result = $this->Database->get('clients',$content)->fetchArray();
				if(!empty($result)){
      ...
      case "divisions":
			case "division":
        $config['type'] = 'text';
				$result = $this->Database->get('divisions',$content)->fetchArray();
				if(!empty($result)){
					$config['content'] = ucwords(str_replace('_',' ',str_replace('.',' ',$result['name'])));
				} else {
					$config['content'] = '';
      ...
      case "contacts":
        $config['type'] = 'text';
				$result = $this->Database->get('contacts',$content)->fetchArray();
				if(!empty($content)){
