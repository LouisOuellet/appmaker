...
class History extends Database{
	...
	public function create($table,$fields){
		...
		$this->Status = $history;
		if($this->Status){ $this->saveTransaction($table, 'create', [], $fields, $results['insert_id'], $results['affected_rows']); }
		return $results;
	}
	...
	public function read($table, $id = null, $field = 'id'){
		...
		if($this->Status){
			if(count($results->all()) > 0){
				$this->saveTransaction($table, 'read', $results->all(), [], $id, count($results->all()));
			}
		}
		return $results;
	}
	...
	public function update($table, $fields, $id, $field = 'id'){
		...
		if($this->Status){
			...
			$this->saveTransaction($table, 'save', $before, $fields, $id, $results);
		}
		return $results;
	}
